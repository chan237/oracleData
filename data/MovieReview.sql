...
CREATE OR REPLACE TRIGGER update_movie_avg
...
BEGIN
    ...
    IF INSERTING OR UPDATING THEN
        UPDATE MOVIES
        SET AVG = (SELECT AVG(RATING) FROM REVIEWS WHERE MOVIE_ID = :NEW.MOVIE_ID)
        WHERE MOVIE_ID = :NEW.MOVIE_ID;
    ELSIF DELETING THEN
        UPDATE MOVIES
        SET AVG = (SELECT AVG(RATING) FROM REVIEWS WHERE MOVIE_ID = :OLD.MOVIE_ID)
...
select * from Movies;
EXECUTE list_movies_by_genre('로멘스');
